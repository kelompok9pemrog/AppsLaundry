...
namespace LaundryApp
{
    ...
    public partial class orderpage : ContentPage
    {
        ...
        private void Btnpay_Clicked(object sender, EventArgs e)
        {
            ...
            koneksi.Open();
            MySqlCommand cmd = new MySqlCommand("select id_jenis,shirt,tshirt,trousers,towel from jenis_service where type = @tipe ", koneksi);

            cmd.Parameters.AddWithValue("tipe", type);
            var rd = cmd.ExecuteReader();
            rd.Read();
            string idjen = rd["id_jenis"].ToString();
            Application.Current.Properties["jenis"] = idjen;
